...
- [ ] Read the completed Slice 1 and Slice 2 plans plus current implementation before changing code. Preserve existing launch/poll/send semantics except where this plan explicitly extends them.
- [ ] Add failing adapter/service tests establishing name validation and initial name propagation:
  - `launch` requires a nonblank `name` in addition to `message`;
  - a valid launch returns/persists the chosen normalized name;
...
- [ ] Add failing contract tests for `list`:
...
- [ ] Add failing contract tests for `complete`:
...
- [ ] Agree on/encode a stable structured result for list, for example:
...
- [ ] Add typed records/DTOs for the new lifecycle data:
...
- [ ] Replace the current single-record lookup with a parent-ledger reconstruction that scans `vigil-launch`, `vigil-turn`, and `vigil-complete` entries for the current parent session file:
...
- [ ] Ensure a completion record dominates later lookup for the same ID. A valid ledger created by Vigil will never append a new turn after completion; defensively avoid treating malformed history as an active runnable child.
- [ ] Keep this session-only and append-only. Do not add a JSON database, in-memory registry, or parent-session mutation/deletion.
- [ ] Add tests for reconstruction across launches, multiple turns, completions, interleaved IDs, and unrelated custom entries.
...
- [ ] Extend child spawning input/CLI construction so only `launch` supplies the normalized required name via Pi’s public `--name` option. Keep session ID, model, cwd, and test session-dir behavior unchanged.
- [ ] Add a narrow child-session naming port/adapter using public Pi session APIs:
...
- [ ] Do not rewrite the child JSONL directly. Do not overwrite a changed name with the launch name.
- [ ] Add deterministic tests using fixtures/fakes for initial name, a changed current name, missing current name, and a naming failure/missing child session.
- [ ] Add a focused CLI-boundary assertion covering `--name` for `launch` and its absence from `send`.
...
- [ ] Add `list` and `complete` to the `vigil` `StringEnum` action schema, plus optional boolean `includeCompleted` compatible with Pi tool schemas.
...
- [ ] Implement `list` service behavior:
...
- [ ] Implement `complete` service behavior in this order:
...
- [ ] Make `poll` recognize a completion record and return a `completed` snapshot while continuing to read the retained child session’s latest response when available.
- [ ] Make `send` explicitly reject a completed ID before any process/session mutation.
- [ ] Format `list` tool text concisely and return `VigilListResult` in `details`; continue returning structured snapshots for launch/poll/send/complete.
- [ ] Update README API, lifecycle, naming, and retention documentation.
...
- [ ] Extend the single existing opt-in acceptance test through the registered extension adapter:
...
- [ ] Retain the current opt-in/auth prerequisite behavior and test model/timeouts.
- [ ] Ensure test teardown, not `complete`, removes only temporary test directories/processes.
- [ ] Run and record:
  - `npm test`;
  - `npm run typecheck`;
...
- [ ] Confirm all default tests remain deterministic and credential-free.
- [ ] Confirm no child JSONL deletion is performed by `complete`; document that Pi’s normal session UI/manual cleanup remains available to the user.
- [ ] Confirm default list excludes completed children and `includeCompleted` includes them without an external registry.
- [ ] Confirm no Slice 4 `search` or unrelated lifecycle APIs were added.
- [ ] Update this plan’s checkboxes, progress notes, assumptions, and deviations; commit plan updates with code/tests.
- [ ] Provide the reviewer: commit SHA, test results, live acceptance status, public API changes, exact completion/name retention behavior, and deviations.
