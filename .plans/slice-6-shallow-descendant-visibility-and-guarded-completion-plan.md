...
- 2026-08-02 (review remediation): Addressed five confirmed review findings via red-green TDD.
  ...
  - Initial authenticated live run exposed a stale synthetic-fixture expectation (`waiting`) for a deliberately missing descendant session; the corrected policy is `unknown`, and parent completion appends its own session-info rename while leaving the synthetic child ledger unchanged.
  - Authenticated live acceptance (`PI_VIGIL_LIVE=1 npm run test:acceptance`): **2/2 passed** after that fixture correction; `npm test` — 208 passed; `npm run typecheck` — clean.
